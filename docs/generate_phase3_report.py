...
cell_bold = ParagraphStyle("CellBold", fontName="Helvetica-Bold", fontSize=9, textColor=NAVY, leading=13)
cell_body = ParagraphStyle("CellBody", fontName="Helvetica", fontSize=8.5, textColor=BLACK, leading=12)
cell_note = ParagraphStyle("CellNote", fontName="Helvetica", fontSize=8.5, textColor=GREY, leading=12)
...
status_partial_s = ParagraphStyle("StatusPartial", fontName="Helvetica-Bold", fontSize=9, textColor=CYAN)
...
def cover_info_table(rows):
    ...
    data = [
        [Paragraph(k, cover_info_label), Paragraph(v, cover_info_value)]
        for k, v in rows
    ]
...
def info_table(rows):
    ...
    data = [
        [
            Paragraph(f"<b>{k}</b>", ParagraphStyle("ik", fontName="Helvetica-Bold", fontSize=9, textColor=NAVY)),
...
def build():
    ...
    story.append(milestone_table([
        ...
        ok("Service model — name, slug, tagline, description, bullet_points, icon_svg, order, is_active"),
        ok("Industry model — name, slug, tagline, description, services_offered, image, order, is_active"),
        ...
        ok("About page — full content template (counter strip, what-we-do, benefit cards, CTA)"),
    ...
    story.append(generic_table(
        ...
        [
            [
                ...
                Paragraph(r[1], ParagraphStyle("cl", fontName="Helvetica", fontSize=8.5, textColor=BLACK, leading=12)),
    ...
    page_rows = [
        ("/", "HomeView", "main", "home.html", "Hero, service cards, 7-service grid, stats, about strip, why-choose, partners, industries"),
        ("/about/", "AboutView", "main", "about.html", "Counter strip, what-we-do, 4 benefit cards, 7-service grid, CTA"),
        ("/services/", "ServicesListView", "services", "list.html", "Banner, all 7 services as list rows, CTA"),
        ("/services/<slug>/", "ServiceDetailView", "services", "detail.html", "Banner, description, bullet list, sidebar CTA, other services panel"),
        ("/industries/", "IndustriesListView", "industries", "list.html", "Career banner, all 6 industry cards, CTA"),
        ("/industries/<slug>/", "IndustryDetailView", "industries", "detail.html", "Banner, check-wrapper, services grid, consultancy CTA"),
    ]
    story.append(generic_table(
        ["URL", "View", "App", "Template", "Content Sections"],
        [
            [
                Paragraph(r[0], ParagraphStyle("ur", fontName="Helvetica-Bold", fontSize=8, textColor=NAVY, leading=11)),
                Paragraph(r[1], ParagraphStyle("vw", fontName="Helvetica", fontSize=8, textColor=BLACK, leading=11)),
                Paragraph(r[2], ParagraphStyle("ap", fontName="Helvetica", fontSize=8, textColor=GREY, leading=11)),
                Paragraph(r[3], ParagraphStyle("tp", fontName="Helvetica", fontSize=8, textColor=BLACK, leading=11)),
                Paragraph(r[4], ParagraphStyle("sc", fontName="Helvetica", fontSize=8, textColor=GREY, leading=11)),
            ]
            for r in page_rows
        ],
    ...
    story.append(generic_table(
        ...
        [
            [
                Paragraph(r[0], cell_bold),
                Paragraph(r[1], ParagraphStyle("ld", fontName="Helvetica", fontSize=8.5, textColor=BLACK, leading=12)),
                Paragraph(r[2], cell_note),
            ]
            for r in lim_rows
        ],
