...
cover_tagline = ParagraphStyle(
    "CoverTagline", fontName="Helvetica", fontSize=12,
    textColor=CYAN, alignment=1, spaceAfter=0,
)
cover_report_label = ParagraphStyle(
    "CoverReportLabel", fontName="Helvetica", fontSize=10,
    textColor=GREY, alignment=1, spaceAfter=4,
)
cover_phase = ParagraphStyle(
    "CoverPhase", fontName="Helvetica-Bold", fontSize=48,
    textColor=NAVY, alignment=1, spaceAfter=0,
)
cover_complete = ParagraphStyle(
    "CoverComplete", fontName="Helvetica-Bold", fontSize=16,
    textColor=NAVY, alignment=1, spaceAfter=0,
)
cover_info_label = ParagraphStyle(
    "CoverInfoLabel", fontName="Helvetica-Bold", fontSize=9, textColor=CYAN,
)
cover_info_value = ParagraphStyle(
    "CoverInfoValue", fontName="Helvetica", fontSize=9, textColor=BLACK,
)
intro_style = ParagraphStyle(
    "Intro", fontName="Helvetica", fontSize=9.5, textColor=NAVY,
...
subsection_title = ParagraphStyle(
    "SubsectionTitle", fontName="Helvetica-Bold", fontSize=10.5,
...
cell_ms = ParagraphStyle(
    "CellMs", fontName="Helvetica", fontSize=9, textColor=BLACK, leading=13,
)
status_ok_s = ParagraphStyle(
    "StatusOk", fontName="Helvetica-Bold", fontSize=9, textColor=GREEN,
)
status_removed = ParagraphStyle(
    "StatusRemoved", fontName="Helvetica-Bold", fontSize=9, textColor=RED,
)
note_italic = ParagraphStyle(
    "NoteItalic", fontName="Helvetica-Oblique", fontSize=9,
...
def info_table(rows):
    data = [[
        Paragraph(f"<b>{k}</b>", ParagraphStyle(
            "ik", fontName="Helvetica-Bold", fontSize=9, textColor=NAVY)),
        Paragraph(v, ParagraphStyle(
            "iv", fontName="Helvetica", fontSize=9, textColor=BLACK)),
    ] for k, v in rows]
...
def build():
    ...
    # ── 1. Executive Summary ───────────────────────────────────────────────────
    story += [
        sp(4), section_box("1.  Executive Summary"), sp(4),
        Paragraph(
    ...
    # ── 2. Phase Milestones ────────────────────────────────────────────────────
    story += [
        sp(4), section_box("2.  Phase Milestones"), sp(4),
        milestone_table([
    ...
    story += [
        sp(4), section_box("3.  Technology Decision — Wagtail → django-unfold"), sp(4),
    ...
    # ── 4. Deliverables ────────────────────────────────────────────────────────
    story += [
        sp(4), section_box("4.  Deliverables"), sp(4),
        ...
        sp(3),
        info_table([
            ("Logo",         "bjp-mark-transparent.png — BJP three-circle mark, "
                             "36 px, transparent background, rendered at top of sidebar"),
            ("Primary color", "#1565C0 (blue) as Unfold primary-500; "
                              "#0D1B4B (navy) as primary-800"),
            ("Sidebar",      "Declarative nav: Dashboard, Website Content "
    ...
    # ── 5. Database Cleanup ────────────────────────────────────────────────────
    story += [
        sp(4), section_box("5.  Database Cleanup"), sp(4),
        Paragraph(
        ...
        generic_table(
            ...
            [
                ...
                [Paragraph("wagtailimages", cell_bold),
                 Paragraph("image, rendition", cell_body)],
                [Paragraph("wagtaildocs / embeds / forms / redirects / search / users", cell_bold),
                 Paragraph(
                     "document, embed, formsubmission, redirect, "
                     "indexentry (+ 5 FTS variants), userprofile",
                     cell_body)],
                [Paragraph("taggit", cell_bold),
                 Paragraph("tag, taggeditem", cell_body)],
                [Paragraph("Total", cell_bold),
                 Paragraph("<b>46 tables dropped</b>", cell_bold)],
            ],
            [50 * mm, 124 * mm],
        ),
        sp(3),
    ...
    # ── 6. Files Changed ──────────────────────────────────────────────────────
    story += [
        sp(4), section_box("6.  Files Changed"), sp(4),
        generic_table(
            ["File", "Action", "Summary"],
            [
                [Paragraph("requirements.txt", cell_bold),
                 Paragraph("Modified", cell_body),
                 Paragraph("Removed wagtail, django-modelcluster, django-taggit; "
                           "added django-unfold==0.92.0", cell_body)],
                [Paragraph("config/settings/base.py", cell_bold),
                 Paragraph("Modified", cell_body),
                 Paragraph("Replaced Wagtail INSTALLED_APPS/MIDDLEWARE/settings with "
                           "Unfold apps + full UNFOLD config block", cell_body)],
                ...
                [Paragraph("apps/core/migrations/0001_*.py", cell_bold),
                 ...
                 Paragraph("RunPython migration — drops 46 Wagtail tables on migrate", cell_body)],
                ...
                [Paragraph("templates/admin/index.html", cell_bold),
                 Paragraph("Created", cell_body),
                 Paragraph("Custom dashboard: KPI cards, status panel, enquiries list", cell_body)],
                [Paragraph("apps/*/wagtail_hooks.py (×4)", cell_bold),
                 Paragraph("Deleted", cell_body),
                 Paragraph("All Wagtail hook files removed from core, services, "
                           "industries, contact", cell_body)],
                ...
                [Paragraph("templates/core/wagtail/ (×2 files)", cell_bold),
                 Paragraph("Deleted", cell_body),
                 Paragraph("Old dashboard_stats.html and dashboard_enquiries.html removed", cell_body)],
                [Paragraph("static/css/wagtail_brand.css", cell_bold),
                 Paragraph("Cleaned", cell_body),
                 Paragraph("Removed Wagtail-specific overrides; reduced to minimal .bjp-sl-img rule", cell_body)],
            ],
            [60 * mm, 22 * mm, 92 * mm],
        ),
    ]
    ...
    # ── 7. Git Commits ────────────────────────────────────────────────────────
    story += [
        sp(4), section_box("7.  Git Commit History"), sp(4),
        generic_table(
            ["Commit", "Type", "Description"],
            [
                [Paragraph("736d018", cell_bold), Paragraph("feat", cell_body),
                 Paragraph("Phase 5 — Wagtail 7.3.1 CMS integration with BJP brand admin", cell_body)],
                [Paragraph("d78db63", cell_bold), Paragraph("feat", cell_body),
                 Paragraph("Add site overview stats panel to dashboard", cell_body)],
                [Paragraph("8204536", cell_bold), Paragraph("feat", cell_body),
                 Paragraph("Redesign dashboard KPI cards — compact premium style", cell_body)],
                [Paragraph("a6808ce", cell_bold), Paragraph("fix", cell_body),
                 Paragraph("Replace Wagtail bird logo with BJP horizontal light logo", cell_body)],
                [Paragraph("529f71c", cell_bold), Paragraph("fix", cell_body),
                 Paragraph("Stop sidebar vanishing — remove MutationObserver, switch to BJP mark logo", cell_body)],
                [Paragraph("3e2dafe", cell_bold), Paragraph("feat", cell_body),
                 Paragraph("Replace Wagtail with django-unfold 0.92.0 — Phase 5 admin", cell_body)],
                [Paragraph("b8843ba", cell_bold), Paragraph("fix", cell_body),
                 Paragraph("Add breathing room between dashboard rows; drop 46 Wagtail DB tables", cell_body)],
                [Paragraph("d16ec90", cell_bold), Paragraph("chore", cell_body),
                 Paragraph("Add migration to auto-drop Wagtail tables on manage.py migrate", cell_body)],
                [Paragraph("f0f70a9", cell_bold), Paragraph("fix", cell_body),
                 Paragraph("Add breathing room between header, KPI cards and enquiry panels", cell_body)],
            ],
            [22 * mm, 18 * mm, 134 * mm],
        ),
    ]
    ...
    # ── 8. Test Results ────────────────────────────────────────────────────────
    story += [
        sp(4), section_box("8.  Test Results"), sp(4),
        info_table([
            ...
            ("Coverage areas", "Contact forms/models/views · Industries models/views · "
                               "Services models/views · Main views"),
            ("Lint",           "ruff check .  →  All checks passed\n"
                               "black --check .  →  74 files unchanged"),
            ("Django check",   "python manage.py check  →  0 errors (6 deployment warnings, expected in dev)"),
        ]),
    ]

    # ── 9. Admin Feature Summary ──────────────────────────────────────────────
    story += [
        sp(4), section_box("9.  Admin Feature Summary"), sp(4),
    ...
    # ── 10. Next Phase ────────────────────────────────────────────────────────
    story += [
        sp(4), section_box("10.  Next Phase — Phase 6: Polish & Launch"), sp(4),
        Paragraph(
        ...
        sp(2),
        milestone_table([
            [Paragraph("Security settings verified (HSTS, CSRF, XSS, SSL redirect)", cell_ms),
             Paragraph("⏳  Pending", ParagraphStyle("pend", fontName="Helvetica",
            ...
            [Paragraph("All images optimised", cell_ms),
             Paragraph("⏳  Pending", ParagraphStyle("pend", fontName="Helvetica",
                                                      fontSize=9, textColor=GREY))],
            [Paragraph("Final security audit (no secrets, no debug, no SQL exposure)", cell_ms),
             Paragraph("⏳  Pending", ParagraphStyle("pend", fontName="Helvetica",
                                                      fontSize=9, textColor=GREY))],
